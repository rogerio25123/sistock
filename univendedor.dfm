object frmvendedor: Tfrmvendedor
  Left = 243
  Top = 120
  Width = 664
  Height = 532
  Caption = 'Vendedores'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  PixelsPerInch = 96
  TextHeight = 13
  object GroupBox1: TGroupBox
    Left = 0
    Top = 8
    Width = 646
    Height = 185
    Caption = ' Cadastro de Vendedor '
    TabOrder = 0
    object Label1: TLabel
      Left = 8
      Top = 24
      Width = 46
      Height = 13
      Caption = 'Vendedor'
    end
    object Label2: TLabel
      Left = 8
      Top = 64
      Width = 31
      Height = 13
      Caption = 'Senha'
    end
    object Label3: TLabel
      Left = 8
      Top = 104
      Width = 53
      Height = 13
      Caption = 'Adm(S/N)?'
    end
    object Label4: TLabel
      Left = 242
      Top = 12
      Width = 53
      Height = 13
      Caption = 'Data Inicial'
    end
    object Label5: TLabel
      Left = 369
      Top = 16
      Width = 48
      Height = 13
      Caption = 'Data Final'
    end
    object Label6: TLabel
      Left = 241
      Top = 51
      Width = 56
      Height = 13
      Caption = 'Comiss'#227'o %'
    end
    object editvendedor: TEdit
      Left = 8
      Top = 40
      Width = 217
      Height = 21
      CharCase = ecUpperCase
      TabOrder = 0
    end
    object editsenha: TEdit
      Left = 8
      Top = 80
      Width = 105
      Height = 21
      CharCase = ecUpperCase
      PasswordChar = '*'
      TabOrder = 1
    end
    object GroupBox2: TGroupBox
      Left = 239
      Top = 91
      Width = 273
      Height = 90
      Caption = 'Pesquisar'
      TabOrder = 5
      object Label12: TLabel
        Left = 72
        Top = 64
        Width = 43
        Height = 13
        Caption = 'Matricula'
      end
      object editpesquisar: TEdit
        Left = 8
        Top = 24
        Width = 169
        Height = 21
        CharCase = ecUpperCase
        TabOrder = 0
      end
      object BitBtn2: TBitBtn
        Left = 184
        Top = 21
        Width = 83
        Height = 29
        Caption = '&Pesquisar'
        TabOrder = 1
        OnClick = BitBtn2Click
        Glyph.Data = {
          
          00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      end
      object BitBtn3: TBitBtn
        Left = 184
        Top = 56
        Width = 83
        Height = 29
        Caption = '&Excluir     '
        TabOrder = 3
        OnClick = BitBtn3Click
        Glyph.Data = {
          
          00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      end
      object editexclui: TEdit
        Left = 120
        Top = 56
        Width = 57
        Height = 21
        CharCase = ecUpperCase
        TabOrder = 2
      end
    end
    object BitBtn1: TBitBtn
      Left = 126
      Top = 80
      Width = 75
      Height = 29
      Caption = '&Gravar'
      TabOrder = 3
      OnClick = BitBtn1Click
      Glyph.Data = {
        
        0000}
      NumGlyphs = 2
    end
    object BitBtn4: TBitBtn
      Left = 126
      Top = 118
      Width = 75
      Height = 29
      Caption = '&Alterar'
      TabOrder = 4
      OnClick = BitBtn4Click
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FF1322BD050DA5FF00FFFF00FF}
    end
    object editregistro: TEdit
      Left = 88
      Top = 16
      Width = 121
      Height = 21
      TabOrder = 6
      Text = 'editregistro'
      Visible = False
    end
    object BitBtn5: TBitBtn
      Left = 558
      Top = 16
      Width = 75
      Height = 70
      Caption = '&Fechar'
      TabOrder = 7
      OnClick = BitBtn5Click
      Glyph.Data = {
        
        0909090909090908080808080808080808080809090909000000}
      Layout = blGlyphTop
    end
    object editadm: TEdit
      Left = 8
      Top = 120
      Width = 57
      Height = 21
      CharCase = ecUpperCase
      TabOrder = 2
      OnKeyPress = editadmKeyPress
    end
    object editporc: TEdit
      Left = 241
      Top = 65
      Width = 57
      Height = 21
      TabOrder = 8
      Text = '1'
      OnKeyPress = editporcKeyPress
    end
    object editdata1: TDateTimePicker
      Left = 241
      Top = 27
      Width = 121
      Height = 21
      Date = 39748.927194189800000000
      Time = 39748.927194189800000000
      TabOrder = 9
    end
    object editdata2: TDateTimePicker
      Left = 369
      Top = 28
      Width = 121
      Height = 21
      Date = 39748.927337963000000000
      Time = 39748.927337963000000000
      TabOrder = 10
    end
    object BitBtn6: TBitBtn
      Left = 306
      Top = 57
      Width = 75
      Height = 29
      Caption = 'Calcular'
      TabOrder = 11
      OnClick = BitBtn6Click
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    end
  end
  object DBGrid1: TDBGrid
    Left = 0
    Top = 200
    Width = 249
    Height = 294
    DataSource = dm2.srcvendedor
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 1
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'registro'
        Width = 50
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'vendedor'
        Width = 138
        Visible = True
      end>
  end
  object DBGrid2: TDBGrid
    Left = 254
    Top = 200
    Width = 392
    Height = 294
    DataSource = srccomissao
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 2
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'vendedor'
        Width = 146
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'totalvendedor'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'comissao'
        Visible = True
      end>
  end
  object sqlcomissao111: TQuery
    OnCalcFields = sqlcomissao111CalcFields
    DatabaseName = 'sistock'
    SQL.Strings = (
      
        'SELECT vendedor , sum( total )  totalvendedor  FROM dadosvendas ' +
        ' where codcli = 0   GROUP BY vendedor')
    Left = 288
    Top = 336
    object sqlcomissao111vendedor: TStringField
      FieldName = 'vendedor'
      Origin = 'SISTOCK."dadosvendas.DB".Vendedor'
      Size = 30
    end
    object sqlcomissao111totalvendedor: TCurrencyField
      FieldName = 'totalvendedor'
      Origin = 'SISTOCK."dadosvendas.DB".Total'
    end
    object sqlcomissao111Comissao: TFloatField
      FieldKind = fkCalculated
      FieldName = 'Comissao'
      DisplayFormat = '0.00'
      Calculated = True
    end
  end
  object srccomissao: TDataSource
    DataSet = sqlcomissao
    Left = 312
    Top = 376
  end
  object sqlcomissao: TZQuery
    Connection = dm2.conexao
    OnCalcFields = sqlcomissaoCalcFields
    SQL.Strings = (
      
        'SELECT vendedor , sum( total )  totalvendedor  FROM dadosvendas ' +
        ' where codcli = 0   GROUP BY vendedor')
    Params = <>
    Left = 280
    Top = 376
    object sqlcomissaovendedor: TStringField
      FieldName = 'vendedor'
      Size = 40
    end
    object sqlcomissaototalvendedor: TFloatField
      FieldName = 'totalvendedor'
      ReadOnly = True
    end
    object sqlcomissaocomissao: TCurrencyField
      FieldKind = fkCalculated
      FieldName = 'comissao'
      currency = False
      Calculated = True
    end
  end
end
